...
public class BinaryUploader {
	...
	public static final Map<String, Object> save(HttpServletRequest request, Map<String, Object> conf) {
		
		boolean isAjaxUpload = request.getHeader( "X_Requested_With" ) != null;

		if (!ServletFileUpload.isMultipartContent(request)) {
			return BaseState.create(false, AppInfo.NOT_MULTIPART_CONTENT);
		}
...
//			String url = OSSFactory.build().upload(file.getBytes(),suffix);
			String url= QiniuUtil.qiniuUpload(file.getBytes(),suffix);
			...
			is.close();*/
			Map<String, Object> state=BaseState.create(true,AppInfo.SUCCESS);
			state.put( "size",file.getSize() );
			state.put( "title", "");//文件名填入此处
			state.put( "url", url);//文件访问的url填入此处
			state.put("type", suffix);
			state.put("original", originFileName + suffix);			

			return state;
		} catch (Exception e) {
			return BaseState.create(false, AppInfo.PARSE_REQUEST_ERROR);
		}
	}
