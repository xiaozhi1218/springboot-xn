...
public class QiniuUtil {
     ...
     * 生成上传凭证，然后准备上传
     */
    private static String accessKey =CloudStorageConfig.getAccessKey();
    private static String secretKey = CloudStorageConfig.getSecretKey();
    private static String bucket = CloudStorageConfig.getBucket();
    private static String origin=CloudStorageConfig.getCdnPrefix();
    private static  Auth auth = Auth.create(accessKey, secretKey);

    public static String qiniuUpload(byte[] data,String fileName){

        //构造一个带指定Zone对象的配置类 zone0华东
        ...
        try {
            ...
            return origin+response.jsonToMap().get("key");
    ...
    public static String delFile(String url){
        ...
        try {
            ...
            System.out.println(response.jsonToMap());
            return "删除成功";
        } catch (QiniuException e) {
            return "删除失败";
        }
...
//        long maxSize = 5242880;
        ...
        extMap.put("image", "gif,jpg,jpeg,png,bmp,apk,ipa");
